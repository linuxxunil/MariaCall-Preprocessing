...
public class Transfer2FSVMDataSet extends Common {
	private DatabaseDriver srcdb = null;
	private DatabaseDriver dstdb = null;

	public Transfer2FSVMDataSet() {
	}
	...
	private void deleteTestingGarbageRows(int id, int winAvg, int kalman,
		...
		try {
			String sql = String.format(
					...
					DatabaseTable.SvmTesting.name, id, winAvg, kalman);
			...
			sql = String.format("DELETE FROM %s WHERE rowid in "
					...
					DatabaseTable.SvmTesting.name, id, winAvg, kalman, rowCount
							- keepLastCount);
			srcdb.delete(sql);
	...
	public void processTraining(String src, int maxID, int garbageRowsCount,
		...
		try {
			Files.copy(new File(src + ".sqlite.orig").toPath(), new File(
					"PC-FSVM/SVM" + src + ".sqlite").toPath());
		} catch (IOException e) {
			e.printStackTrace();
		}

		srcdb = new SqliteDriver("PC-FSVM/SVM" + src + ".sqlite");
		srcdb.onConnect();
		// 產生Filter後的Database
		...
		for (int k = 0; k < 4; k++) { // handle 4 mode
			...
			dstdb = new SqliteDriver(String.format(
					"PC-FSVM/SVMtraining%02d.sqlite", k));
			dstdb.onConnect();
		...
		for (int k = 0; k < 4; k++) { // handle 4 mode
			...
			dstdb = new SqliteDriver(String.format(
					"PC-FSVM/SVMtesting%02d.sqlite", k));
			dstdb.onConnect();
			dstdb.createTable(DatabaseTable.Ibeacon.create());
	...
	public void processTraining2(String src, int maxID, int garbageRowsCount,
		...
		// 產生Filter後的Database
		ResultSet rs;
		double[][][][] rssi = new double[maxID][macSet.length][4][garbageRowsCount
				+ trainingCount + testingCount]; // BeaconNmber/Mode/DataSet
		...
		for (int id = 0; id < maxID; id++) { // Area
			for (int m = 0; m < macSet.length; m++) { // Beacon
				String sql = String.format("SELECT * FROM Ibeacon WHERE "
						+ "id=\'%d\' AND device=\'%s\' AND "
						+ "winAvg=\'%d\' AND Kalman=\'%d\'", id, macSet[m], 0,
						0);
				...
				try {
					...
					while (rs.next()) { // dataset
						...
						rssi[id][m][2][i] = exeWinAvg(winAvg,
								rssi[id][m][0][i], i);
						rssi[id][m][3][i] = exeWinAvg(winAvg,
								rssi[id][m][1][i], i);
						i++;
					}
				} catch (Exception e) {
		...
		for (int mode = 0; mode < 4; mode++) {// Mode
			dstTrainingDb = new SqliteDriver(String.format(
					"PC-FSVM/SVMtraining%02d.sqlite", mode));
			dstTrainingDb.onConnect();
			dstTrainingDb.createTable(DatabaseTable.Ibeacon.create());

			dstTestingDb = new SqliteDriver(String.format(
					"PC-FSVM/SVMtesting%02d.sqlite", mode));
			dstTestingDb.onConnect();
			dstTestingDb.createTable(DatabaseTable.Ibeacon.create());

			for (i = garbageRowsCount; i < garbageRowsCount + trainingCount
					+ testingCount; i++) {
				for (int id = 0; id < maxID; id++) {// Area
					for (int m = 0; m < macSet.length; m++) { // Beacon
						if (i < garbageRowsCount + trainingCount) {
							insertTraining(dstTrainingDb, id, macSet[m],
									String.valueOf(rssi[id][m][mode][i]), "0",
	...
	public void processTraining3(String src, int maxID, int garbageRowsCount,
			int trainingCount, int testingCount, String[] macSet) {
		DatabaseDriver dstTrainingDb = null;
		DatabaseDriver dstTestingDb = null;
		int[] mode1 = { 0, 0, 1, 1 };
		...
		// 產生Filter後的Database
		ResultSet rs;
		double[][][][] rssi = new double[maxID+1][macSet.length][4][garbageRowsCount
				+ trainingCount + testingCount]; // BeaconNmber/Mode/DataSet

		int i;
		for (int id = 1; id < maxID+1; id++) { // Area
			for (int m = 0; m < macSet.length; m++) { // Beacon
				String sql = String.format("SELECT * FROM Ibeacon WHERE "
						+ "id=\'%d\' AND device=\'%s\' AND "
						+ "winAvg=\'%d\' AND Kalman=\'%d\'", id, macSet[m], 0,
	...
	public void calRecalling(String src, int maxID) {
		...
		double accuracy = 0;
		double total2 = 0;
		srcdb = new SqliteDriver("PC-FSVM/" + src + ".sqlite");
		srcdb.onConnect();

		for (int id = 0; id < maxID; id++) { //
			String sql = String.format("SELECT * FROM %s WHERE id=%d",
					DatabaseTable.SvmTesting.name, id);
			correct = total = 0;
			try {
				ResultSet rs = srcdb.select(sql);
				while (rs.next()) {
			...
			System.out.println("Accuracy[" + id + "]="
					...
					+ total + ")");
			writeToTxt("PC-FSVM/" + src + ".txt",
					((double) correct / total * 100) + "%");

		}
		System.out.println("TotalAccuracy=" + (total2 / maxID));
		writeToTxt("PC-FSVM/" + src + ".txt", ((double) (total2 / maxID)) + "%");
		srcdb.close();
	}
	...
	public void calRecallingErrorBlack(String src, int maxID) {
		...
		for (int id = 0; id < maxID; id++) { //
			String sql = String.format(
					...
					DatabaseTable.SvmTesting.name, id, id);
	...
	public void processRecalling(String src, int maxID, int keepLastCount) {
		...
		try {
			Files.copy(new File(src + ".sqlite.svm.recalling.orig").toPath(),
					new File("PC-FSVM/SVM" + src + ".sqlite.recalling")
							.toPath());
		} catch (IOException e) {
			e.printStackTrace();
		}

		srcdb = new SqliteDriver("PC-FSVM/SVM" + src + ".sqlite.recalling");
		srcdb.onConnect();
		// 產生Filter後的Database
		...
		for (int k = 0; k < 4; k++) { // handle 4 mode
										// (None/WinAvg/Kalman/Hybrid
			dstdb = new SqliteDriver(String.format(
					"PC-FSVM/SVMrecalling%02d.sqlite", k));
			dstdb.onConnect();
			...
			for (int id = 0; id < maxID; id++) { //
				String sql = String.format("SELECT * FROM %s WHERE "
						...
						DatabaseTable.SvmTesting.name, id, mode1[k], mode2[k],
						keepLastCount);
		...
		try {
			while (rs.next()) {
				...
				if (++i % macSet.length == 0) {
					int id = rs.getInt("id");
					line = id + input;
					writeToTxt("PC-FSVM/" + src + ".txt", line);
					input = "";
					j = 0;
				}
			}
	...
	public static void main(String[] args) {
		...
		Transfer2FSVMDataSet ps = new Transfer2FSVMDataSet();
		...
		for (int i = 0; i < 4; i++) {
			ps.transferToTxt(String.format("SVMtraining%02d", i), areaNumber, macSet);
			ps.transferToTxt(String.format("SVMtesting%02d", i),areaNumber, macSet);
		}
		*/
		...
		for(int i=0;i<3; i++) {
			ps.calRecalling(String.format("SVMrecalling%02d",i),areaNumber);
			System.out.println("================================");
			ps.calRecallingErrorBlack(String.format("SVMrecalling%02d",i),areaNumber);
		}
